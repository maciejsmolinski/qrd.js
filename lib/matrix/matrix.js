;(function() {
   ...
   * ::Matrix (<Array:points>)
   *
   * Returns Matrix instance, might be loaded with predefined points and relations if specified.
   *
   * Sample Usage:
   *   new Matrix() // => Matrix
   *   new Matrix({ points: [ point, point ], relations: [ relation, relation ] }) // => Matrix
   *
   * @param {Object}  options  Configuration object that can contain @points and @relations
   */
  function Matrix (options) {
    this.points    = [];
    this.relations = [];
    if (options && options.points)    { this.addPoints(options.points); }
    if (options && options.relations) { this.addRelations(options.relations); }
  }
  ...
  if (!Matrix.name) { Matrix.name = 'Matrix'; }
